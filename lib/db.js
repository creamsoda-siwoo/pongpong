...
if (!global.__mockDb) {
  ...
  // Safe file load (ignored in read-only serverless environment if not present)
  try {
...
// Atomic file write (guarded for Vercel read-only serverless filesystem)
export function saveLocalDb() {
  try {
    ...
    // Vercel serverless environment is read-only, safely ignore file write errors
  }
}
...
// Supabase Client with Vercel Environment Variable Fallbacks
let supabase = null;
let isMockDb = true;

const sbUrl = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
const sbKey = process.env.SUPABASE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
